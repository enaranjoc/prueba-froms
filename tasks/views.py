...
@login_required
def tasks(request):
    tasks = Task.objects.filter(user=request.user, datecompleted__isnull=True, status=True)
    return render(request, 'tasks.html', {'tasks':tasks})

@login_required
def tasks_completed(request):
    tasks = Task.objects.filter(user=request.user, datecompleted__isnull=False, status=True).order_by('-datecompleted')
    return render(request, 'tasks.html', {'tasks':tasks})
...
def task_detail(request, task_id):
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request, 'detail_task.html',{'task':task, 'form':form, 'error':'Error al actualizar la tarea'})
